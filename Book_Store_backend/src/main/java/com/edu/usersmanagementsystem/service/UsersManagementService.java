...
public class UsersManagementService {
	...
	public void initializeDefaultAdmin() {
		...
		Optional<OurUsers> adminUser = usersRepo.findByEmail(defaultEmail);
		if (adminUser.isEmpty()) {
			OurUsers defaultAdmin = new OurUsers();
			defaultAdmin.setEmail(defaultEmail);
			defaultAdmin.setCity("Mumbai");
			defaultAdmin.setRole("ADMIN");
			defaultAdmin.setName("admin");
			defaultAdmin.setPassword(passwordEncoder.encode("admin"));

			// Save the default admin user
			usersRepo.save(defaultAdmin);
    ...
    public ReqRes login(ReqRes loginRequest){
        ...
        try {
            ...
            response.setCartIdRetrive(user.getId());
    ...
    public ReqRes selfRegister(ReqRes registrationRequest){
        ReqRes resp = new ReqRes();
        ...
        try {
            ...
            if (ourUser.getRole() == null || ourUser.getRole().isEmpty()) {
                ...
                ourUser.setRole(ourUser.getRole());
            }
            ourUser.setName(registrationRequest.getName());
            ourUser.setPassword(passwordEncoder.encode(registrationRequest.getPassword()));
            OurUsers ourUsersResult = usersRepo.save(ourUser);
    ...
    public ReqRes getUsersByIdForProfile(Integer id) {
        ReqRes reqRes = new ReqRes();
        try {
            OurUsers usersById = usersRepo.findById(id).orElseThrow(() -> new RuntimeException("User Not found"));
            reqRes.setOurUsers(usersById);
